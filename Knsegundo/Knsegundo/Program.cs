...
namespace Ki
{
    class Program
    {
        ...
        public static int verbos(List<String> palavrasKlingon)
        {
            ...
            return listaSbs.Count();
        }

        static void primeirapessoa()
        {
            var verbos = "jzfcchbs bjqdhkps jlmtdcdk qkwmdxlf xmjlklzl wqhvvfdl zdcrscjs wjfmtngs zmprxdms cpltlszl mttrtqqs rjrfsgzl xknsclbk gwxlzgfw wnslrsfw cbttjszl zlgwfhsw ncnhxnwk nqdtvvpl cxdhlsns gmfdrhws npdgdlsf dljnvlzw ntqfwvws pmjnsvjf ftnrfrkl gpkxpzqs pjlcmcxf pqspncnk fkmlmqtk";
            var verbosKlingon = verbos.Split(' ').ToList();

            var sbs2 = "";
            var listaSbs2 = new List<string>();



            foreach (var pk2 in verbosKlingon)
            {
                sbs2 = pk2.Substring(0, 1).ToUpper();
                if (sbs2 != "S" && sbs2 != "L" && sbs2 != "F" && sbs2 != "W" && sbs2 != "K")
                {
                    Console.Write(pk2 + " --- ");
                    listaSbs2.Add(sbs2);
            ...
            Console.WriteLine(listaSbs2.Count());
