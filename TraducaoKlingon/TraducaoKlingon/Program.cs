...
namespace Ki
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(Preposicoes(palavrasKlingon)); 

        }
        public static int Preposicoes(List<String> palavrasKlingon)
        {
            palavrasKlingon.RemoveAll(x => x.Count() != 3);

            palavrasKlingon.RemoveAll(x => x.Contains('d'));
            ...
            foreach (var pk in palavrasKlingon)
            {
                ...
                sbs = pk.Substring(2).ToUpper();

                if (sbs != "S" && sbs != "L" && sbs != "F" && sbs != "W" && sbs != "K")
                {
                    Console.Write(pk + " - ");
                    listaSbs.Add(sbs);
